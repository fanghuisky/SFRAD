...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        # 分割
        seg_image = segmentation.copy()
        seg_image[seg_image>threshold] = 255
        seg_image[seg_image<=threshold] = 0
        seg_image = seg_image[:,:, np.newaxis].repeat(3,2)

        ### cv2 save
        image = image.transpose(1, 2, 0)
        cv2_img = cv2.cvtColor(np.asarray(image),cv2.COLOR_RGB2BGR)
        cv2_mask = cv2.cvtColor(np.asarray(mask.transpose(1, 2, 0)),cv2.COLOR_RGB2BGR)*255
        cam_image, heatmap = gen_cam(image, segmentation)
        save_image = np.concatenate((cv2_img, cv2_mask, seg_image, cam_image), axis=1)
        cv2.imwrite(savename[:-4]+'.jpg', save_image)
